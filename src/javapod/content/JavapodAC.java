...
import javapod.content.Javapodattribute;

import static mindustry.Vars.tilesize;
import static mindustry.Vars.world;

public class JavapodAC extends AttributeCrafter {
    public Attribute attribute = Javapodattribute.Zinc;
    public int radius = 0;
    public boolean conf = true;
    public float eff;
    public float confCount;
    public Color radColor = Color.white;
    public JavapodAC(String name){
        super(name);
    }
        ...
        for(int ax = tx - radius; ax < tx + radius + size; ax++){
            for(int ay = ty - radius; ay < ty + radius + size; ay++){
                ...
                if (!(((ax >= tx)&&(ax < tx+size))&&((ay >= ty)&&(ay < ty+size)))) {
                    if (conf && t.block() instanceof JavapodAC) confCount++;
                }
            }
        }
